...
# Sidebar Menu
# Added "Dashboard" to the menu list
menu = st.sidebar.selectbox("Main Menu", ["Dashboard", "View Books", "Manage Books", "Manage Borrowers", "Borrow/Return"])
...
if menu == "Dashboard":
    ...
    if borrowers:
        # Calculate fees for all records
        fees = []
        for b in borrowers:
            try:
                days = int(b[3])
                fee = max(0, days - 7) * 1.0
                fees.append(fee)
            except:
                fees.append(0.0)
        ...
        total_sum = sum(fees)
        avg_fee = total_sum / len(fees) if fees else 0
        total_borrowers = len(borrowers)

        # Display using Streamlit Metrics
        col1, col2, col3 = st.columns(3)
        col1.metric("Total Borrowers", total_borrowers)
        col2.metric("Total Late Fees (Sum)", f"RM{total_sum:.2f}")
        col3.metric("Average Late Fee", f"RM{avg_fee:.2f}")

        st.divider()
        st.subheader("Fee Breakdown")
        df_fees = pd.DataFrame(borrowers, columns=["Student ID", "Name", "Book ID", "Days"])
        # Add a calculated column to the display table
        df_fees['Current Fee (RM)'] = fees
        st.dataframe(df_fees, use_container_width=True)
    else:
        st.info("No borrower data available to calculate statistics.")
...
# 4. Borrow/Return 
    ...
    with col1:
        ...
        # Ask for expected days
        days_input = st.number_input("Borrowing Duration (Days)", min_value=1, value=7)
        if st.button("Process Loan"):
            borrowers = read_file(BORROWERS_FILE)
            for b in borrowers:
                if b[0] == student_id:
                    b[2], b[3] = book_id, str(days_input)
            write_file(BORROWERS_FILE, borrowers)
        ...
        if st.button("Process Return"):
            ...
            for b in borrowers:
                if b[0] == r_sid and b[2] == r_bid:
                    ...
                    if late_fee > 0:
                        st.error(f"Late Fee: RM{late_fee}.00")
                    else:
                        st.success("Returned on time!")
                    # Clear the book but keep the days for the dashboard stats
                    b[2] = "None"
            ...
            if found:
                ...
                st.error("No matching loan record found.")